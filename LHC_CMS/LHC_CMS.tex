...
This active alignment algorithm was implemented in the CMS tracker Prompt Calibration Loop (PCL) for the start of the 2015 run of the LHC. The Prompt Calibration Loop is a collection of routines that are designed to monitor and automatically adjust parameters to optimize the performance of the the CMS tracking system. The alignment routine developed during this thesis work monitors and adjusts the 36 alignment parameters that describe the pixel detector global parameters. The monitored parameters are the positions $\left(x, y, z\right)$ and rotations $\left(\theta_{x}, \theta_{y}, \theta_{z}\right)$ of the two BPIX half-barrels and the four half-cylinders of the FPIX system\footnote{Two half-cylinders each for FPIX+ and FPIX-.}. If a sufficient shift in one of these parameters is seen\footnote{In this case, ``sufficient" implies the parameter shifts between a pre-set minimum and maximum window and the significance of that shift is larger than a pre-set amount.} then the database of tracker conditions is updated. These consistent updates allow CMS to continue operating at peak performance and avoids rerunning computationally expensive data processing routines to correct the data after the fact. 

This algorithm is also useful to determine the best position parameters immediately after the startup of the collider. This use case was demonstrated the week of July 6th, 2015. This week marked the beginning of data-taking for the CMS experiment at a new higher center of mass energy, $\sqrt{s} = \unit{13}{\TeV}$ with the full magnetic field\footnote{Coincidentally, also coincided with the week this document was submitted to the review committee.}. Within 48 hours of the beginning of this data-taking this alignment workflow was run and produced results. These showed a movement of the Pixel detector compared to the positions that had been determined before the magnet was turned on. The magnitude of these initial shifts can be seen in figure \ref{fig:PCL_DQM_first}. This plot shows the detected shifts in the BPIX and FPIX half-barrels and half-cylinders in positions $\left(x, y, z\right)$ and rotations $\left(\theta_{x}, \theta_{y}, \theta_{z}\right)$. Note that the axis range for the $z$ and $\theta_{z}$ shifts are twice ($\unit{200}{\micro\meter}$) that of their $x$ and $y$ counterparts. Further, this algorithm was able to correct this movement and produce a geometry that was more accurate and useful for the very next data. Figures \ref{fig:PCL_DQM_2} confirms that the next set of data would not trigger the creation of a new geometry because the residual differences are small. 

\begin{figure}
\begin{center}
\includegraphics[width=0.9\linewidth]{{LHC_CMS/251134_Detail}.pdf}
\caption[Shifts in Pixel Tracker large scale structures as detected by the Alignment in Prompt Calibration Loop using very first runs of CMS data at $\sqrt{s} = \unit{13}{\TeV}$ with the full magnetic field ($\unit{3.8}{\tesla}$). Green histograms show shifts, with black error bars as uncertainties. Red entries and error bars represent cutoffs in movement and uncertainty that are considered significant, respectively. Please note the axis range for the $z$ and $\theta_{z}$ shifts are twice ($\unit{200}{\micro\meter}$) that of their $x$ and $y$ counterparts.]{Shifts in Pixel Tracker large scale structures as detected by the Alignment in Prompt Calibration Loop using very first runs of CMS data at $\sqrt{s} = \unit{13}{\TeV}$ with the full magnetic field ($\unit{3.8}{\tesla}$). Green histograms show shifts, with black error bars as uncertainties. Red entries and error bars represent cutoffs in movement and uncertainty that are considered significant. Please note the axis range for the $z$ and $\theta_{z}$ shifts are twice ($\unit{200}{\micro\meter/\micro\radian}$) that of their $x$ and $y$ counterparts.}
\label{fig:PCL_DQM_first}
\end{center}
\end{figure}

\begin{figure}
\begin{center}
\includegraphics[width=0.9\linewidth]{{LHC_CMS/Expert_251168}.pdf}
\caption[Shifts in Pixel Tracker large scale structures after correction for shifts detected by the Alignment in Prompt Calibration Loop using very first runs of CMS data at $\sqrt{s} = \unit{13}{\TeV}$ with the full magnetic field ($\unit{3.8}{\tesla}$). Green histograms show shifts, with black error bars as uncertainties. Red entries and error bars represent cutoffs in movement and uncertainty that are considered significant, respectively. Please note the axis range for the $z$ and $\theta_{z}$ shifts are identical to that of their $x$ and $y$ counterparts.]{Shifts in Pixel Tracker large scale structures after correction for shifts detected by the Alignment in Prompt Calibration Loop using very first runs of CMS data at $\sqrt{s} = \unit{13}{\TeV}$ with the full magnetic field ($\unit{3.8}{\tesla}$). Green histograms show shifts, with black error bars as uncertainties. Red entries and error bars represent cutoffs in movement and uncertainty that are considered significant. Please note the axis range for the $z$ and $\theta_{z}$ shifts are identical to that of their $x$ and $y$ counterparts.}
\label{fig:PCL_DQM_2}
\end{center}
\end{figure}

Figure \ref{fig:Validations} show the accuracy of the alignment object created by this workflow by comparing three iterations (1) before starting to take new data (blue), (2) before PCL alignment but after some alignment (green), and (3) after PCL alignment (black). The top two plots show the number of tracks vs $\chi^{2}$(left) and the $\chi^{2}$ per degrees of freedom distribution (right). One can see that indeed the PCL alignment produces better fits to the tracks. The middle and bottom plots are distributions of the medians of residuals (DMRs). These show that the median residual in the $x$(left) and $y$(right) direction for each module in BPIX (middle) and FPIX(bottom) decreases after an alignment is performed. Similarly, one can see the impact that correct alignment of the BPIX and FPIX have on the other tracker subdetectors by observing the benefit of the PCL workflow in TIB DMR, figure \ref{fig:TIBvalidation}.

\begin{figure}
\begin{center}
\includegraphics[width=0.45\linewidth]{{LHC_CMS/Ntrack_vs_chi2_mine}.pdf}
\includegraphics[width=0.45\linewidth]{{LHC_CMS/chi2_per_Dof_mine}.pdf} \\
\includegraphics[width=0.45\linewidth]{{LHC_CMS/DmedianR_BPIX_plain}.eps}
\includegraphics[width=0.45\linewidth]{{LHC_CMS/DmedianYR_BPIX_plain}.eps} \\
\includegraphics[width=0.45\linewidth]{{LHC_CMS/DmedianR_FPIX_plain}.eps}
\includegraphics[width=0.45\linewidth]{{LHC_CMS/DmedianYR_FPIX_plain}.eps} \\
\caption[Comparison of three iterations (1) before starting to take new data (blue),  (2) before PCL alignment but after some alignment (green), and  (3) after PCL alignment (black) of first CMS $\sqrt{s} = \unit{13}{\TeV}$ data with the full magnetic field ($\unit{3.8}{\tesla}$). (top left) Number of tracks vs $\chi^{2}$ probability. (top right) $\chi^{2}$ per degrees of freedom distribution. (middle) Distributions of the medians of residuals for BPIX modules in $x$ (left) and $y$ (right). (bottom) Distributions of the medians of residuals for BPIX modules in $x$ (left) and $y$ (right).]{Comparisons before any alignment (red), before PCL alignment (green), and after PCL alignment (black) of first CMS $\sqrt{s} = \unit{13}{\TeV}$ data with the full magnetic field ($\unit{3.8}{\tesla}$). (top left) Number of tracks vs $\chi^{2}$ probability. (top right) $\chi^{2}$ per degrees of freedom distribution. (middle) Distributions of the medians of residuals for BPIX modules in $x$ (left) and $y$ (right). (bottom) Distributions of the medians of residuals for BPIX modules in $x$ (left) and $y$ (right).}
...
\begin{figure}
\begin{center}
\includegraphics[width=0.45\linewidth]{{LHC_CMS/DmedianR_TIB_plain}.eps}
\caption[Comparison of three iterations (1) before starting to take new data (blue), (2) before PCL alignment but after some alignment (green), and (3) after PCL alignment of pixel large structures (black) of first CMS $\sqrt{s} = \unit{13}{\TeV}$ data with the full magnetic field ($\unit{3.8}{\tesla}$). The impact of PCL pixel alignment can be seen on the residuals of the Silicon Strip Tracker Inner Barrel (TIB), while all Silicon Strip parameters are kept fixed..]{Comparison of three iterations (1) before starting to take new data (blue), (2) before PCL alignment but after some alignment (green), and (3) after PCL alignment of pixel large structures (black) of first CMS $\sqrt{s} = \unit{13}{\TeV}$ data with the full magnetic field ($\unit{3.8}{\tesla}$). The impact of PCL pixel alignment can be seen on the residuals of the Silicon Strip Tracker Inner Barrel (TIB), while all Silicon Strip parameters are kept fixed.}
\label{fig:TIBvalidation}
\end{center}
\end{figure}

The effectiveness of this project (long term) is illustrated from similar algorithm that was used at the end of the 2012 run after a large shift in the pixel barrel position was seen. Figure \ref{fig:2011PixelShift} shows the longitudinal shift between the two half-shelf of the BPIX as measured with the primary vertex residuals. The previous monitoring procedure was implemented farther `downstream' in the computing chain than the current implementation.
