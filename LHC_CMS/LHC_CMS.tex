...
\begin{figure}
\begin{center}
\includegraphics[width=.89\linewidth]{{LHC_CMS/cms_onion}.pdf}
...
Figure \ref{fig:Validations} shows the accuracy of the alignment object created by this workflow by comparing three iterations (1) before starting to take new data (blue), (2) before PCL alignment but after some alignment with a different magnetic field configuration (black), and (3) after PCL alignment (magenta). The top and bottom plots are distributions of the medians of residuals (DMRs). These show that the median residual in the $x$(left) and $y$(right) direction for each module in BPIX (top) and FPIX(bottom) decreases after an alignment is performed. Similarly, one can see the impact that correct alignment of the BPIX and FPIX have on the other tracker subdetectors by observing the benefit of the PCL workflow in TIB DMR, figure \ref{fig:TIBvalidation} \cite{CMS-DP-2015-029}.

\begin{figure}
\begin{center}
\includegraphics[width=0.45\linewidth]{{LHC_CMS/3.8Tcollisions_DmedianR_BPIX_plain}.pdf}
\includegraphics[width=0.45\linewidth]{{LHC_CMS/3.8Tcollisions_DmedianYR_BPIX_plain}.pdf} \\
\includegraphics[width=0.45\linewidth]{{LHC_CMS/3.8Tcollisions_DmedianR_FPIX_plain}.pdf}
\includegraphics[width=0.45\linewidth]{{LHC_CMS/3.8Tcollisions_DmedianYR_FPIX_plain}.pdf} \\
\caption[Comparison of three iterations (1) before starting to take new data (blue),  (2) before PCL alignment but after some alignment (black), and  (3) after PCL alignment (magenta) of first CMS $\sqrt{s} = \unit{13}{\TeV}$ data with the full magnetic field ($\unit{3.8}{\tesla}$). (top) Distributions of the medians of residuals for BPIX modules in $x$ (left) and $y$ (right). (bottom) Distributions of the medians of residuals for BPIX modules in $x$ (left) and $y$ (right).]{Comparison of three iterations (1) before starting to take new data (blue),  (2) before PCL alignment but after some alignment (black), and  (3) after PCL alignment (magenta) of first CMS $\sqrt{s} = \unit{13}{\TeV}$ data with the full magnetic field ($\unit{3.8}{\tesla}$). (top) Distributions of the medians of residuals for BPIX modules in $x$ (left) and $y$ (right). (bottom) Distributions of the medians of residuals for BPIX modules in $x$ (left) and $y$ (right) \cite{CMS-DP-2015-029}.}
...
\begin{figure}
\begin{center}
\includegraphics[width=0.45\linewidth]{{LHC_CMS/3.8Tcollisions_DmedianR_TIB_plain}.pdf}
\caption[Comparison of three iterations (1) before starting to take new data (blue), (2) before PCL alignment but after some alignment (black), and (3) after PCL alignment of pixel large structures (magenta) of first CMS $\sqrt{s} = \unit{13}{\TeV}$ data with the full magnetic field ($\unit{3.8}{\tesla}$). The impact of PCL pixel alignment can be seen on the residuals of the Silicon Strip Tracker Inner Barrel (TIB), while all Silicon Strip parameters are kept fixed.]{Comparison of three iterations (1) before starting to take new data (blue), (2) before PCL alignment but after some alignment (black), and (3) after PCL alignment of pixel large structures (magenta) of first CMS $\sqrt{s} = \unit{13}{\TeV}$ data with the full magnetic field ($\unit{3.8}{\tesla}$). The impact of PCL pixel alignment can be seen on the residuals of the Silicon Strip Tracker Inner Barrel (TIB), while all Silicon Strip parameters are kept fixed \cite{CMS-DP-2015-029}.}
